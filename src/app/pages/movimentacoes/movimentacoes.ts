...
export class MovimentacoesComponent implements OnInit {
  ...
  private calcularMetricas(): void {
    ...
    this.produtosSaida = Math.floor(this.produtosEntrada * 0.3);
  }
  ...
  private simularMovimentacoesFromProdutos(): Movimentacao[] {
    ...
    this.produtos.forEach(produto => {
      ...
      if (produto.estoque && produto.estoque > 5) {
        const saidaQuantidade = Math.floor((produto.estoque * 0.3) + 1);
        movimentacoes.push({
          id: idCounter++,
          produtoNome: produto.nome,
          tipo: 'saida',
          quantidade: saidaQuantidade,
          data: new Date(Date.now() - Math.random() * 3 * 24 * 60 * 60 * 1000).toISOString(),
          usuario: nomeUsuario
        });
      }
    });
